...
class TaskController extends Controller
{
    ...
    {
        $query = Task::orderByRaw(Task::PRIORITY_ORDER . ' DESC')->orderBy('due_date', 'asc');

        if ($request->has('status')) {
            $status = $request->query('status');
            if (!in_array($status, ['pending', 'in_progress', 'done'])) {
                return response()->json(['message' => 'Invalid status. Use: pending, in_progress, done.'], 422);
            }
        ...
        if ($tasks->isEmpty()) {
            return $this->successResponse('No tasks found.', []);
        }
    ...
    public function updateStatus(Request $request, int $id): JsonResponse
    {
        ...
        if ($nextStatus === null) {
            return response()->json(['message' => 'Task is already done. No further transitions allowed.'], 422);
        }

        $task->status = $nextStatus;
        $task->save();

        return $this->successResponse("Status updated to '{$nextStatus}'.", $task);
    }
    ...
    public function report(Request $request): JsonResponse
    {
        ...
        if ($validator->fails()) {
            return response()->json(['message' => 'Provide a valid date: YYYY-MM-DD.', 'errors' => $validator->errors()], 422);
        }
